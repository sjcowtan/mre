...
```{r sort_polygon_file}
# arrange(match()) allows sorting into an arbitrary order
polygons_nested_df <- polygons_nested_df %>%
  arrange(match(Name, levels(sea_areas_sf$name)))
```

```{r name_seas}
# Changes sea region to last one in file, which is smallest
